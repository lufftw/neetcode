...
- **Version History** (main pipeline only): `tools/mindmaps/ai-markmap-agent/outputs/versions/v1/` (for tracking changes)
...
- **Path Resolution**: Relative paths are always resolved relative to the script directory (`tools/mindmaps/ai-markmap-agent/`), not the current working directory
...
- Default directory: `tools/mindmaps/core/meta/`
...
- Relative paths are always resolved relative to the script directory (`tools/mindmaps/ai-markmap-agent/`), not the current working directory
...
See [Post-Processing Links Documentation](https://github.com/lufftw/neetcode/blob/main/tools/mindmaps/ai-markmap-agent/docs/post-processing-links.md) for details.
