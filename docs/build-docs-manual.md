...
### Note: Difference Between `mkdocs build` and `mkdocs build -c`

- **`mkdocs build`**
  - Builds the documentation into the `site/` directory as specified in `mkdocs.yml`.
  - It does **not** delete any files in `site/` that are not generated by this build.
  - Old files from previous builds may remain in `site/` if they are not overwritten by new content.

- **`mkdocs build -c`** (or `--clean`)
  - Cleans the `site/` directory **before** building.
  - Deletes everything inside `site/` before adding newly generated files.
  - Ensures that only the latest documentation files exist in `site/`, preventing issues from leftover or outdated files.

**Recommendation:**  
Use `mkdocs build -c` for a clean, reliable build, especially when adding, removing, or renaming documentation files.
