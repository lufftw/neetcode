...
- **How to use**
  ...
  - **Quick router (decision guide)**
    - Need **contiguous subarray/substring** optimal under a constraint? → `SubstringSlidingWindow` (fixed-size vs variable-size; counting vs sum)
    - Need **two ends / sorted scanning / in-place compaction**? → **Family: Two-Pointer Techniques** → `TwoPointersTraversal`
    - Need **cycle/midpoint** on linked structure or functional graph? → **Family: Two-Pointer Techniques** → `FastSlowPointers`
    - Need **reorder by predicate** or **Kth**? → **Family: Two-Pointer Techniques** → `TwoPointerPartition` (or `HeapTopK` for streaming)
    - Need **merge sorted sequences**? → `MergeSortedSequences` / `KWayMerge`
    - Need **monotone predicate**? → `BinarySearchBoundary` (index boundary vs answer space)
    - Need **shortest steps/time** on unweighted graph/grid? → `GridBFSMultiSource`
- **Progress tracker**
...
## ⚡ API Kernel: `HashMapLookup` (Complement / counting lookup)
- **Kernel Contract**
  ...
  - **Failure modes**: needs hashable key; worst-case hash collisions; “return indices” depends on whether you mutate/sort
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when you must preserve original order under sorting-based transforms (unless you store indices)
...
- **Where you’ll use this at work**
  - Log scanning / token streams (e.g., “smallest span covering keywords”)
...
- **Patterns**
  ...
  - **At most K distinct** (`sliding_window_at_most_k_distinct`) — maximize
    - [LeetCode 340](https://leetcode.com/problems/longest-substring-with-at-most-k-distinct-characters/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0340_longest_substring_with_at_most_k_distinct.py)
  - **Frequency cover / exact match (variable-size)** (`sliding_window_freq_cover`) — minimize/exists/all
    - [LeetCode 76](https://leetcode.com/problems/minimum-window-substring/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0076_minimum_window_substring.py)
  - **Fixed-size window (exact multiset over length |t|)** (`sliding_window_fixed_size`) — exists/all
    - [LeetCode 567](https://leetcode.com/problems/permutation-in-string/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0567_permutation_in_string.py)
    - [LeetCode 438](https://leetcode.com/problems/find-all-anagrams-in-a-string/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0438_find_all_anagrams_in_a_string.py)
...
- **Comparison table (mental model)**
  - | Problem | Invariant | State | Window | Goal |
    ...
    | [LeetCode 3](https://leetcode.com/problems/longest-substring-without-repeating-characters/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0003_longest_substring_without_repeating_characters.py)
    | [LeetCode 340](https://leetcode.com/problems/longest-substring-with-at-most-k-distinct-characters/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0340_longest_substring_with_at_most_k_distinct.py)
    | [LeetCode 76](https://leetcode.com/problems/minimum-window-substring/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0076_minimum_window_substring.py)[c] ≥ need[c]` (equiv: `formed == required`) | need/have + formed | variable | min |
    | [LeetCode 567](https://leetcode.com/problems/permutation-in-string/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0567_permutation_in_string.py)
    | [LeetCode 438](https://leetcode.com/problems/find-all-anagrams-in-a-string/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0438_find_all_anagrams_in_a_string.py)
...
- **Trade-offs (common interviewer follow-ups)**
  ...
  - Tracking `formed/required` avoids $O(\Sigma)$ checks per step; naive “covers” checks often TLE or bug on duplicates

---

## 🧩 Family: Two-Pointer Techniques
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when objective/constraints are non-monotone on pointer moves (often indicates need for DP/graph/search)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
...
- **Patterns**
  - **Opposite pointers: maximize function** (`two_pointer_opposite_maximize`)
    - [LeetCode 11](https://leetcode.com/problems/container-with-most-water/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0011_container_with_most_water.py)
    - **Lemma (dominance)**: If `height[L] ≤ height[R]`, any container using `L` with some `R' < R` cannot beat current width while min-height is bounded by `height[L]`; therefore incrementing `L` cannot discard the optimal.
...
- **Key table (choose the right sub-pattern)**
  - | Sub-pattern | When it applies | Typical invariant | Example |
    ...
    | Writer | in-place filter/dedup | `arr[:write]` is exactly “kept from arr[:read]” | [LeetCode 26](https://leetcode.com/problems/remove-duplicates-from-sorted-array/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0026_remove_duplicates_from_sorted_array.py)
...
- **Kernel Contract**
  ...
  - **Failure modes**: requires deterministic “next”; can’t detect cycles if graph branches (need visited set)
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when nodes have multiple outgoing edges (use DFS/BFS + visited)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
...
- **Kernel Contract**
  ...
  - **Failure modes**: quickselect worst-case $O(n^2)$ with bad pivots; mutates array (may violate “don’t modify input”)
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when you need stability (relative order preserved) unless you use stable partition (extra space)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
...
- **Where you’ll use this at work**
  ...
  - Two-way “stream join” style logic when inputs are ordered
- **Compositions**
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when inputs are unsorted (sort first or use heap)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
...
- **Two production-grade strategies**
  - **Min-heap** (`merge_k_sorted_heap`) — $O(N \log K)$, great when K is large and lists are uneven / streaming
  - **Divide & conquer** (`merge_k_sorted_divide`) — $O(N \log K)$, often faster constants + less heap churn
- **Where you’ll use this at work**
...
- **Must-know (🔥) / High-frequency (⭐)**
  ...
  - **High-frequency (⭐)**
    - ⭐ [LeetCode 4](https://leetcode.com/problems/median-of-two-sorted-arrays/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0004_median_of_two_sorted_arrays.py)(hybrid; see BinarySearchBoundary partition)
- **Problems**
  - [LeetCode 23](https://leetcode.com/problems/merge-k-sorted-lists/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0023_merge_k_sorted_lists.py)
...
- **Where you’ll use this at work**
  ...
  - capacity planning style “min feasible X” (throughput, resource sizing)
- **Compositions**
  - Binary search on answer + greedy/DP feasibility checker
  - Partition-index binary search in sorted arrays (order statistics)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
    - 🔥 [LeetCode 4](https://leetcode.com/problems/median-of-two-sorted-arrays/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0004_median_of_two_sorted_arrays.py)(partition index)
- **Patterns**
  ...
  - **Binary search on partition (index) in sorted arrays** (`binary_search_partition_index`)
    - [LeetCode 4](https://leetcode.com/problems/median-of-two-sorted-arrays/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0004_median_of_two_sorted_arrays.py)
    - **Invariant**: choose partition `i` in A (and `j` in B) so left parts have size `k`; maintain `A[i-1] ≤ B[j]` and `B[j-1] ≤ A[i]` as feasibility; predicate is monotone in `i`.
...
- **Kernel Contract**
  - **Inputs**: stream or batch array; comparator for “best”; K
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work when you need total order of all items (sort) or worst-case linear-time guarantees (quickselect caveats)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
    - 🔥 [LeetCode 215](https://leetcode.com/problems/kth-largest-element-in-an-array/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0215_kth_largest_element_in_an_array.py)
- **Patterns**
...
- **Must-know (🔥) / High-frequency (⭐)**
  ...
  - **High-frequency (⭐)**
    - ⭐ [LeetCode 2](https://leetcode.com/problems/add-two-numbers/description/) | [Solution](https://github.com/lufftw/neetcode/blob/main/solutions/0002_add_two_numbers.py)(related family, different operation)
- **Patterns**
...
- **Kernel Contract**
  ...
  - **Failure modes**: if edges have weights ≠ 1, need Dijkstra; revisiting without visited/dist checks breaks correctness
...
- **Works when… / Doesn’t work when…**
  ...
  - Doesn’t work for weighted edges (use priority queue / Dijkstra)
- **Must-know (🔥) / High-frequency (⭐)**
  - **Must-know (🔥)**
...
## ⚡ API Kernel: `MonotonicStack` (Next greater/smaller + range dominance)
- **Kernel Contract**
...
- **Where you’ll use this at work**
  - Time-series “next time higher/lower” queries, alerting, span computations
...
## ⚡ API Kernel: `TreeTraversalDFS` (Recursive / iterative DFS on trees)
- **Kernel Contract**
...
## ⚡ API Kernel: `TreeTraversalBFS` (Level-order traversal)
- **Kernel Contract**
...
- **Core invariant**: queue holds exactly the next frontier; processing one “level batch” corresponds to a fixed depth
- **Where you’ll use this at work**
...
## ⚡ API Kernel: `DPSequence` (1D DP on sequences)
- **Kernel Contract**
...
## ⚡ API Kernel: `DPInterval` (DP on ranges / intervals)
- **Kernel Contract**
