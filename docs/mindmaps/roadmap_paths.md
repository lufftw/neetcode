---
title: Learning Roadmaps - Structured Problem Paths
markmap:
  colorFreezeLevel: 3
  maxWidth: 400
---

# Learning Roadmaps

Curated problem sequences for different learning goals.

## Blind 75 (4 problems)

*The famous curated list for FAANG interviews*

### 🟢 Easy (1)

- 🟢 [LeetCode 1 - Two Sum](../../solutions/0001_two_sum.py)

### 🟡 Medium (1)

- 🟡 [LeetCode 3 - Longest Substring Without Repeating Characters](../../solutions/0003_longest_substring_without_repeating_characters.py)

### 🔴 Hard (2)

- 🔴 [LeetCode 23 - Merge k Sorted Lists](../../solutions/0023_merge_k_sorted_lists.py)
- 🔴 [LeetCode 76 - Minimum Window Substring](../../solutions/0076_minimum_window_substring.py)

## Graph BFS Path (1 problems)

*Breadth-first search patterns*

### 🟡 Medium (1)

- 🟡 [LeetCode 994 - Rotting Oranges](../../solutions/0994_rotting_oranges.py)

## Grind 75 (3 problems)

*Updated Blind 75 with flexible scheduling*

### 🟢 Easy (1)

- 🟢 [LeetCode 1 - Two Sum](../../solutions/0001_two_sum.py)

### 🟡 Medium (1)

- 🟡 [LeetCode 3 - Longest Substring Without Repeating Characters](../../solutions/0003_longest_substring_without_repeating_characters.py)

### 🔴 Hard (1)

- 🔴 [LeetCode 76 - Minimum Window Substring](../../solutions/0076_minimum_window_substring.py)

## LeetCode Top 100 (5 problems)

*Most liked problems on LeetCode*

### 🟢 Easy (1)

- 🟢 [LeetCode 1 - Two Sum](../../solutions/0001_two_sum.py)

### 🟡 Medium (2)

- 🟡 [LeetCode 2 - Add Two Numbers](../../solutions/0002_add_two_numbers.py)
- 🟡 [LeetCode 3 - Longest Substring Without Repeating Characters](../../solutions/0003_longest_substring_without_repeating_characters.py)

### 🔴 Hard (2)

- 🔴 [LeetCode 4 - Median of Two Sorted Arrays](../../solutions/0004_median_of_two_sorted_arrays.py)
- 🔴 [LeetCode 23 - Merge k Sorted Lists](../../solutions/0023_merge_k_sorted_lists.py)

## NeetCode 150 (12 problems)

*Comprehensive coverage of essential patterns*

### 🟢 Easy (1)

- 🟢 [LeetCode 1 - Two Sum](../../solutions/0001_two_sum.py)

### 🟡 Medium (6)

- 🟡 [LeetCode 2 - Add Two Numbers](../../solutions/0002_add_two_numbers.py)
- 🟡 [LeetCode 3 - Longest Substring Without Repeating Characters](../../solutions/0003_longest_substring_without_repeating_characters.py)
- 🟡 [LeetCode 209 - Minimum Size Subarray Sum](../../solutions/0209_minimum_size_subarray_sum.py)
- 🟡 [LeetCode 438 - Find All Anagrams in a String](../../solutions/0438_find_all_anagrams_in_a_string.py)
- 🟡 [LeetCode 567 - Permutation in String](../../solutions/0567_permutation_in_string.py)
- 🟡 [LeetCode 994 - Rotting Oranges](../../solutions/0994_rotting_oranges.py)

### 🔴 Hard (5)

- 🔴 [LeetCode 4 - Median of Two Sorted Arrays](../../solutions/0004_median_of_two_sorted_arrays.py)
- 🔴 [LeetCode 23 - Merge k Sorted Lists](../../solutions/0023_merge_k_sorted_lists.py)
- 🔴 [LeetCode 25 - Reverse Nodes in k-Group](../../solutions/0025_reverse_nodes_in_k_group.py)
- 🔴 [LeetCode 51 - N-Queens](../../solutions/0051_n_queens.py)
- 🔴 [LeetCode 76 - Minimum Window Substring](../../solutions/0076_minimum_window_substring.py)

## Sliding Window Path (6 problems)

*Master the sliding window technique*

### 🟡 Medium (5)

- 🟡 [LeetCode 3 - Longest Substring Without Repeating Characters](../../solutions/0003_longest_substring_without_repeating_characters.py)
- 🟡 [LeetCode 209 - Minimum Size Subarray Sum](../../solutions/0209_minimum_size_subarray_sum.py)
- 🟡 [LeetCode 340 - Longest Substring with At Most K Distinct Characters](../../solutions/0340_longest_substring_with_at_most_k_distinct.py)
- 🟡 [LeetCode 438 - Find All Anagrams in a String](../../solutions/0438_find_all_anagrams_in_a_string.py)
- 🟡 [LeetCode 567 - Permutation in String](../../solutions/0567_permutation_in_string.py)

### 🔴 Hard (1)

- 🔴 [LeetCode 76 - Minimum Window Substring](../../solutions/0076_minimum_window_substring.py)
