...
使用 big_O 函式庫方法自動估算演算法時間複雜度。

### 設計理念

**簡單且通用** - 只需在 generator 中額外提供一個函式：

| 函式 | 用途 | 必要 |
|------|------|------|
| `generate(count, seed)` | 隨機測資，用於功能測試 | ✅ 必要 |
| `generate_for_complexity(n)` | 控制大小的測資，用於複雜度估算 | 可選 |

估算器內部使用 **Mock stdin** 方法：
- ✅ 通用 - 適用於任何有 `solve()` 函式的解法
- ✅ 無 subprocess 開銷
- ✅ 維持 stdin 抽象設計
...
| `generate_for_complexity(n)` | ✅ | 接受大小 `n` 並回傳測資輸入的函式 |

### 適合的題目類型

並非所有題目都適合做時間複雜度估算。估算在以下情況效果最好：

| ✅ 適合 | ❌ 不適合 |
|---------|----------|
| 輸入大小 `n` 可連續變化（10, 100, 1000...） | 輸入大小有硬性限制（如 n ≤ 9） |
| 執行時間隨輸入大小變化 | 執行時間被固定開銷主導 |
| 線性、對數、多項式複雜度 | 階乘/指數複雜度且 n 很小 |

**範例：**

| 題目 | 適合？ | 原因 |
|------|--------|------|
| Two Sum | ✅ | n 可以是 10 ~ 10000，O(n) 變化明顯 |
| Longest Substring | ✅ | 字串長度可大幅變化 |
| Merge k Sorted Lists | ✅ | 總元素數 N 可變化 |
| N-Queens (0051) | ❌ | n ≤ 9（階乘爆炸），無法有意義地變化大小 |
| Rotting Oranges (0994) | ❌ | 網格大小受限，BFS 時間被網格結構主導 |
| Sudoku Solver | ❌ | 固定 9x9 網格，回溯複雜度 |

> **提示**：只在 `n` 能從小（10）到大（1000+）有意義變化的 generator 中加入 `generate_for_complexity(n)`。

### 向後兼容

- **解答檔案**：無需變更（必須有 `solve()` 函式）
- **現有 generator**：無需變更即可繼續運作
- **新功能**：加入 `generate_for_complexity(n)` 以啟用估算
