...
# 指定目標
python tools/mindmaps/generate_mindmaps_ai.py --goal interview      # 面試準備
python tools/mindmaps/generate_mindmaps_ai.py --goal systematic     # 系統學習
python tools/mindmaps/generate_mindmaps_ai.py --goal quick_review   # 快速複習
python tools/mindmaps/generate_mindmaps_ai.py --goal pattern_mastery # 模式掌握

# 指定主題
python tools/mindmaps/generate_mindmaps_ai.py --topic sliding_window
python tools/mindmaps/generate_mindmaps_ai.py --topic dp

# 指定風格
python tools/mindmaps/generate_mindmaps_ai.py --style creative
python tools/mindmaps/generate_mindmaps_ai.py --style minimal
python tools/mindmaps/generate_mindmaps_ai.py --style academic
```

### 配置檔案

編輯 `tools/mindmaps/generate_mindmaps_ai.toml` 可自訂：
- 使用的 LLM 模型
- 輸出目錄和檔名
- 要包含的 ontology 資料
- 題目篩選條件
...
2. **複製生成的 prompt**：
   - 打開 `tools/mindmaps/prompts/generated/mindmap-prompt.md`
...
編輯 `tools/mindmaps/prompts/system-prompt.md` 可修改 LLM 的角色設定和行為規則。
...
編輯 `tools/mindmaps/prompts/prompts_config.yaml` 可修改：
